...
def main_gui(run_func):
    ...
    layout = [
        ...
        [
            sg.Frame('上传文件夹', [
                [sg.Button('监听中', key='-LISTEN_DIR-'),
                 sg.FolderBrowse('选择', enable_events=True, change_submits=True, key='-CHANGE_LISTEN_DIR-',
                                 target='-CHANGE_LISTEN_DIR-'),
                 sg.Text(str(task_manager.UPLOAD_DIR.absolute()), key='-UPLOAD_DIR_LABEL-')]
            ]),

            sg.Frame('失败文件夹', [
                [
                    sg.FolderBrowse('选择', enable_events=True, change_submits=True, key='-CHANGE_FAILED_DIR-',
                                    target='-CHANGE_FAILED_DIR-'),
                    sg.Text(str(task_manager.FAILED_DIR.absolute()), key='-FAILED_DIR_LABEL-'),
                ]
            ]),

            sg.Frame('重复文件夹', [
                [
                    sg.FolderBrowse('选择', enable_events=True, change_submits=True, key='-CHANGE_REPEAT_DIR-',
                                    target='-CHANGE_REPEAT_DIR-'),
                    sg.Text(str(task_manager.REPEAT_DIR.absolute()), key='-REPEAT_DIR_LABEL-'),
                ]
            ]),
            ...
            sg.Frame('操作', [
                [
                    sg.Checkbox("显示浏览器", default=task_manager.show_browser, enable_events=True,
                                key="-SHOW_BROWSER-"),
                    sg.Text('线程数量:'),
                    sg.InputText(str(task_manager.THREAD_COUNT), key='-THREAD_COUNT-', size=(5, 1))
                ],
                [
                    sg.Push(),
                    sg.Button('开始'),
        ...
        while True:
            ...
            if event in (sg.WINDOW_CLOSED, '退出'):
                ...
                window['-LISTEN_DIR-'].update(
                    text='监听中' if task_manager.LISTEN_DIR else '未监听')
            elif event == '-CHANGE_LISTEN_DIR-':
                task_manager.change_listen_dir(pathlib.Path(values[event]))
                window['-UPLOAD_DIR_LABEL-'].update(str(task_manager.UPLOAD_DIR.absolute()))
            elif event == '-CHANGE_FAILED_DIR-':
                task_manager.change_failed_dir(pathlib.Path(values[event]))
                window['-FAILED_DIR_LABEL-'].update(str(task_manager.FAILED_DIR.absolute()))
            elif event == '-CHANGE_REPEAT_DIR-':
                task_manager.change_failed_dir(pathlib.Path(values[event]))
                window['-REPEAT_DIR_LABEL-'].update(str(task_manager.REPEAT_DIR.absolute()))
